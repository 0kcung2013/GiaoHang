-- The customer starts the first assignment. After a driver transfers an
-- unclaimed offer, that rejected driver may trigger the same deterministic
-- server-side assignment for the next candidate. The caller cannot choose the
-- next driver.
...
DECLARE
  ...
  v_actor_is_rejected_driver boolean := false;
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE driver_profile.user_id = v_actor_id
      AND COALESCE(v_order.rejected_by, '[]'::jsonb)
          ? v_actor_id::text
  )
  INTO v_actor_is_rejected_driver;

  IF v_actor_id IS DISTINCT FROM v_order.customer_id
     AND NOT v_actor_is_rejected_driver THEN
