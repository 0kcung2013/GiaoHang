-- Default dispatch stays deliberately local: the automatic system only offers
-- orders to drivers at most 3 km from the pickup point. FreePick is a separate,
-- driver-initiated path and never steals an active offer.
  ...
  WITH candidate_distances AS (
    ...
    WHERE auth.uid() IS NOT NULL
      ...
      AND public.ST_DWithin(
        ...
        LEAST(GREATEST(radius_meters, 1), 50000)
      )
  )
  SELECT
    candidate.id,
...
BEGIN
  ...
  IF offer_order.assignment_expires_at <= current_time THEN
    ...
    SET assignment_timed_out_at = current_time,
  ...
  IF offer_order.offered_driver_id IS NOT NULL
     AND offer_order.offer_expires_at > current_time THEN
...
BEGIN
  ...
  END IF;
  RETURN private.dispatch_next_order_offer(p_order_id, p_radius_meters);
END;
$$;
...
BEGIN
  ...
  PERFORM private.dispatch_next_order_offer(p_order_id, 3000);
  RETURN new_deadline;
END;
$$;
