-- Offer the nearest eligible driver after excluding previous rejections.
-- Assignment and its status log remain in one PostgreSQL transaction.
...
DROP FUNCTION IF EXISTS public.find_nearest_drivers(
  double precision,
  double precision,
  double precision,
  integer
  ...
  LOOP
    ...
    EXCEPTION
      WHEN unique_violation THEN
        -- Another transaction reserved this driver for a different active
        -- order. Continue with the next deterministically ranked candidate.
        CONTINUE;
    END;
  END LOOP;
