-- FreePick is a manual opt-in path. Keep the RPC boundary aligned with the
-- app control so an outdated or modified client cannot search or claim an
-- order farther than 4 km from the driver's latest location.
